---
description: >-
  How Rezerve uses DeFi and smart contracts to reach it's goal of building the
  largest community-owned onchain treasury
---
...
1. Onchain Bonds — short-term discounted ETH accumulation.
...
Rezerve then uses this newly acquired ETH and onchain treasury to provide liquidity to the RZR token, allowing investors to get enough liquidity to enter/exit from the ecosystem further increasing the loop.
...
Rezerve issues onchain convertible bonds. These are tokenized instruments that allows the protocol to raise debt and give lenders the option to convert their debt into RZR tokens at a later price.
...
* **ETH and network tokens Accumulation:** Every bond purchase increases Rezerve’s ETH treasury and onchain network tokens for broader exposure.
...
RZR is first deposited into lending protocols as collateral. The Rezerve protocol then borrows stablecoins or other liquid assets at low interest rates. The borrowed capital is used to purchase more ETH and other network tokens like SOL, BNB, increasing the reserve size.
...
* **Growth Acceleration**: Allows the protocol to accumulate network tokens faster than organic inflows alone.
